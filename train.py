...
# Load class sequences into np arrays
jabs = []
nones = []
uppercuts = []
...
for num_uppercuts in range(0, 121):
    uppercuts.append(np.load(f'data/uppercut/uppercut_{num_uppercuts}.npy'))
uppercuts = np.array(uppercuts)

# Combine arrays and create corresponding labels
samples = np.concatenate((uppercuts, jabs, nones))
labels = np.concatenate((np.full(121, 2), np.ones(103), np.zeros(102)))

# Randomize order
indices = np.random.permutation(326)
samples = samples[indices]
labels = labels[indices]

# Convert from np arrays to PyTorch tensors, split into training and val
training_samples = torch.from_numpy(samples[:260]).float().squeeze(1)
val_samples = torch.from_numpy(samples[260:]).float().squeeze(1)
training_labels = torch.from_numpy(labels[:260]).long()
val_labels = torch.from_numpy(labels[260:]).long()
...
model = LSTM(34, 32, 1, 3) # 3 -> jab, uppercut, none
